...
"""
PX4 Offboard Pose control in ROS2. The controller publishes attitude setpoints (attitude and thrust) given desired position and yaw commands. 
This controller assumes the vehicle is underactuated. The attitude control is done in PX4.
...
from px4_offboard_ros2.common import thrust_to_attitude

class OffboardPoseControl(Node):
    def __init__(self):
        super().__init__('pose_control_py')
        ...
        self.hover_thrust_estimate = 0.3 # using PX4 estimator
        ...
        # controller params
        # tuning parameters for tilted_hex
        self.kp = np.array([0.1,0.1,0.1])
        self.ki = np.array([0.001,0.001,0.02])
        self.kd = np.array([0.09,0.09,0.02])
    ...
    def engage_offboard_mode(self) -> None:
        ...
        self.position_control.match_state(self.position,self.vel)
        self.get_logger().info(f"{self.position_control.print_state()}")

        self.publish_px4_command(
            VehicleCommand.VEHICLE_CMD_DO_SET_MODE, param1=1.0, param2=6.0)
        self.get_logger().info("Switching to offboard mode (resetting state)")
    ...
    def control_loop(self) -> None:
        ...
        if self.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            ...
            yaw_sp = self.att_sp_rpy[2] # in radians
